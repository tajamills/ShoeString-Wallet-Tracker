...
// Help Modal with Telegram Instructions
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="pt-4 border-t border-white/10">
            <p className="text-white/40 text-sm">
              Need more help? Contact support at support@cryptobagtracker.io
            </p>
          </div>
        </div>
      </div>
...
export const AppLayout = ({ 
  ...
  useEffect(() => {
    const fetchSubscription = async () => {
      try {
        const response = await axios.get(`${API}/alerts/subscription`, { 
          headers: getAuthHeader() 
        });
  ...
  return (
    <div className="min-h-screen bg-[#0d0d14] flex">
      {/* Sidebar */}
      <div className="w-56 bg-[#12121a] border-r border-white/5 flex flex-col shrink-0">
        {/* Logo */}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Footer */}
        <footer className="h-12 border-t border-white/5 flex items-center justify-between px-6 text-white/30 text-xs shrink-0">
...
// Alerts Content
export const AlertsContent = ({ getAuthHeader }) => {
  ...
  const connectTelegram = async () => {
    ...
    try {
      ...
      setSuccess('Telegram connected! Check your Telegram for a confirmation.');
      setTelegramChatId('');
      fetchData();
      setTimeout(() => setSuccess(''), 5000);
    } catch (err) {
      setError(err.response?.data?.detail || 'Failed to connect. Make sure you started a chat with @cryptobagtrackerbot first.');
    } finally {
  ...
  return (
    <div className="p-6">
      ...
      {/* Alerts List */}
      {alerts.length === 0 ? (
        <div className="bg-[#1a1a2e] border border-white/10 rounded-xl py-12 text-center mb-6">
              ...
              <div key={alert.alert_id} className="bg-[#1a1a2e] border border-white/10 rounded-xl p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {alert.note && <p className="text-white/40 text-xs mt-1">Note: {alert.note}</p>}
                  </div>
