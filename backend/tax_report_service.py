...
class TaxReportService:
    ...
    def generate_form_8949_csv(
        ...
        try:
            ...
            unique_assets = list(set(g.get('asset', symbol) for g in filtered_gains))
            asset_label = unique_assets[0] if len(unique_assets) == 1 else f"Multiple ({len(unique_assets)} assets)"
            
            # Write header information
            output.write('IRS Form 8949 - Sales and Other Dispositions of Capital Assets\n')
            output.write(f'Generated by Crypto Bag Tracker\n')
            output.write(f'Wallet Address: {address}\n')
            output.write(f'Asset: {asset_label}\n')
            output.write(f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n')
            ...
            for gain in filtered_gains:
                description = f"{gain.get('amount', 0):.8f} {gain.get('asset', symbol)}"
